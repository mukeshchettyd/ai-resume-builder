...
function ResumePreview({ data, template = 'classic' }) {
    ...
    return (
        <div className={shellClass} id="resume-preview">
                        ...
                        <div className="resume-entry" key={entry.id}>
                            <div className="resume-entry-header">
                                ...
                                <div className="resume-entry-links">
                                    {entry.githubUrl && (
                                        <a href={entry.githubUrl} target="_blank" rel="noopener noreferrer" className="resume-link-icon" title="GitHub">
                                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22"></path></svg>
                                        </a>
                                    )}
                                    {entry.liveUrl && (
                                        <a href={entry.liveUrl} target="_blank" rel="noopener noreferrer" className="resume-link-icon" title="Live Demo">
                            ...
                            {entry.techStack && entry.techStack.length > 0 && (
                                <div className="resume-tech-stack">
                                    {entry.techStack.map((tech, i) => (
                                        <span key={i} className="resume-skill-tag resume-skill-tag--small">{tech}</span>
                                    ))}
                                </div>
                            )}
                        </div>
            ...
            {/* ---- Skills ---- */}
            {((typeof skills === 'string' && skills) || (typeof skills === 'object' && (skills.technical?.length > 0 || skills.soft?.length > 0 || skills.tools?.length > 0))) && (
                <div className="resume-section">
